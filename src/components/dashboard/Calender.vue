<template>
	...
	<FullCalendar :options="calendarOptions" :key="calendarIndex" />
</template>
...
const calendarOptions = ref({
	...
	eventMouseEnter: function (info) {
		var tooltip = new boosted.Tooltip(info.el, {
			animation: true,
			html: true,
			container: "body",
			title: `<div>
...
const fetchEvents = async () => {
	const { length } = await getEvents();

	const { events } = await getEvents(null, {
		limit: length,
	});

	allEvents.value = formatEvents(events);
};
