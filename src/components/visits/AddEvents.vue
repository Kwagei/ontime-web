<template>
    ...
    <div id="visit-view" class="d-flex flex-column container">
        ...
        >
            <form class="row g-3" style="width: 110rem">
...
const dataTableOptions = ref({
    ...
    ajax: {
        ...
        dataSrc: (json) => {
            ...
            // ensure event listeners are set for check in button on each row
...
const participantDetail = async (id) => {
    ...
    if (!visitorData) {
        ...
        // display error message in modal if something went wrong
        if (insertedVisitorData.result.status != 201) {
            alert.value.message = insertedVisitorData.result.message;
    ...
    // show belongings modal after we confirmed that there was not any error
    // creating the visitor nor checking if they exists
