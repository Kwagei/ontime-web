<template>
	<div
		class="mt-1 form-control input w-50"
		style="margin: auto; padding: 5rem 5rem; border-radius: 0.5rem"
	>
		<h1 class="text-center display-1">Ontime</h1>
		<div
			class="py-2 mb-3 text-center rounded"
			style="background-color: rgba(34, 135, 34, 0.2)"
		>
			...
			<div class="">
				...
				>
					...
					<div
						...
						class="h-100 px-2 d-flex justify-content-center align-items-center password-visibility"
					>
						<Icons v-if="showPassword" v-model:icon="visionIcon" />
						<Icons v-if="hidePassword" v-model:icon="hideIcon" />
					</div>
				</div>
			</div>
...
const togglePassword = (event) => {
	const input = event.target.closest(".input-group").children[0];
